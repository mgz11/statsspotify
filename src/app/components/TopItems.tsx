...
export default function TopItems({ items, searchType }: TopItemsProps) {
	...
	return (
		<div className="flex flex-col items-center">
			<h2 className="text-xl font-medium mb-4">Your Top {searchType.charAt(0).toUpperCase() + searchType.slice(1)}:</h2>
			<ul className="space-y-4 w-full">
					...
					{items.slice(0, 3).map((item, index) => {
						...
						return (
							...
							>
									...
									{images[0] && (
										<Image
											...
											*/
											...
											className="rounded shadow object-cover w-[400px] h-[400px]"
											quality={100}
											priority
											unoptimized
										/>
								...
								<a href={item.external_urls.spotify} target="_blank" rel="noopener noreferrer" className="mt-4">
									<Image
				...
				<div className="flex flex-col space-y-4">
					{items.slice(3).map((item, index) => {
						const images = getImages(item);
						return (
							<li key={index + 3} className="flex items-center p-4 bg-gray-500 rounded shadow text-sm md:text-xl">
								<p className="text-lg font-semibold mr-4 w-6 text-center">{index + 4}</p>
								{images[0] && (
								...
								<div className="ml-4">
									<p className="font-medium truncate w-24 md:w-full">{item.name}</p>
									{item.artists && searchType === "tracks" && (
